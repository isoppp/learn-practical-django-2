...
class UserManager(BaseUserManager):
    ...
    def create_superuser(self, email, password, **extra_fields):
        ...
        if extra_fields.get("is_staff") is not True:
            raise ValueError("Superuser must hav is_staff=True")
...
class Address(models.Model):
    SUPPORTED_CONTRIES = (("uk", "United Kingdom"), ("us", "United States of America"))
    ...
    country = models.CharField(max_length=3, choices=SUPPORTED_CONTRIES)
...
class Basket(models.Model):
    ...
    def create_order(self, billing_address, shipping_address):
        ...
        logger.info("Created order with id=%d and lines_count=%d", order.id, count)
...
class Order(models.Model):
    ...
    @property
    def summary(self):
        products_counts = self.lines.values("product__name").annotate(c=Count("product__name"))
        pieces = []
        for pc in products_counts:
            pieces.append("%s x %s" % (pc["c"], pc["product__name"]))
        return ", ".join(pieces)

    @property
    def total_price(self):
        res = self.lines.aggregate(total_price=Sum("product__price"))
