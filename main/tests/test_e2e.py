    ...
    def test_product_page_switches_images_correctly(self):
        ...
        for fname in ["cb1.jpg", "cb2.jpg", "cb3.jpg"]:
            with open(f"main/fixtures/cb/{fname}", "rb") as f:
                image = models.ProductImage(product=product, image=ImageFile(f, name=fname))
                image.save()

        self.selenium.get("%s%s" % (self.live_server_url, reverse("product", kwargs={"slug": "cathedral-bazaar"})))

        current_image = self.selenium.find_element_by_css_selector(".current-image > img:nth-child(1)").get_attribute(
            "src"
        )
